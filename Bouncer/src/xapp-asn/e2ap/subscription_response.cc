    ...
    for(unsigned int i = 0; i < ref_notadmitted_action_array->size(); i ++){
      ...
      switch (cause)
      {
        ...
        case Cause_PR_e2Node:
          ie_not_admitted_list[i].value.choice.RICaction_NotAdmitted_Item.cause.choice.e2Node = (*ref_notadmitted_action_array)[i].get_sub_cause();
          break;
        default:
    ...
    switch (memb_ptr->id)
    {
      ...
      case (ProtocolIE_ID_id_RICactions_NotAdmitted):
        ...
        for (int index = 0; index < ric_not_admitted_action_list->list.count; index++)
        {
          ...
          case Cause_PR_e2Node:
            sub_cause = item->value.choice.RICaction_NotAdmitted_Item.cause.choice.e2Node;
            break;
...
void subscription_response::set_fields_unsuccess( subscription_response_failure_helper &helper) {
  ...
  RANfunctionID_t *ranfunction_ie = &ies_ranfunc->value.choice.RANfunctionID;
  *ranfunction_ie = helper.get_func_id();
  ASN_SEQUENCE_ADD(&ric_subscription_failure->protocolIEs, &(IE_Failure_array[ie_index]));

  ie_index = 2;
  RICsubscriptionFailure_IEs_t *ies_cause = &IE_Failure_array[ie_index];
  ies_cause->criticality = Criticality_reject;
  ies_cause->id = ProtocolIE_ID_id_Cause;
  ies_cause->value.choice.Cause = helper.get_cause();
  ASN_SEQUENCE_ADD(&ric_subscription_failure->protocolIEs, &(IE_Failure_array[ie_index]));

  RICsubscriptionFailure_IEs_t *ies_crit = &IE_Failure_array[ie_index];
  ie_index = 3;
  ies_crit->criticality = Criticality_ignore;
  ies_crit->id = ProtocolIE_ID_id_CriticalityDiagnostics;
  if (helper.get_crit_diagnostics()) {  // criticality diagnostics is an optional ie
    ies_crit->value.choice.CriticalityDiagnostics = *(helper.get_crit_diagnostics());
  } else {
    ies_crit->value.present = RICsubscriptionFailure_IEs__value_PR_NOTHING;
  }
  ASN_SEQUENCE_ADD(&ric_subscription_failure->protocolIEs, &(IE_Failure_array[ie_index]));

}

void subscription_response:: get_fields(UnsuccessfulOutcome_t * unsuccess_msg,  subscription_response_failure_helper & dout)
{
  ...
  for (int edx = 0; edx < unsuccess_msg->value.choice.RICsubscriptionFailure.protocolIEs.list.count; edx++)
  {
    ...
    {
      case (ProtocolIE_ID_id_RICrequestID):
        dout.set_request_id(memb_ptr->value.choice.RICrequestID);
        break;

      case (ProtocolIE_ID_id_RANfunctionID):
        dout.set_func_id(memb_ptr->value.choice.RANfunctionID);
        break;

      case (ProtocolIE_ID_id_Cause):
        dout.set_cause(memb_ptr->value.choice.Cause);
        break;

      case (ProtocolIE_ID_id_CriticalityDiagnostics):
        dout.set_crit_diagnostics(&memb_ptr->value.choice.CriticalityDiagnostics);
        break;
    }
  }
